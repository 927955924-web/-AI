...
class AIService:
    ...
    def _build_retrieval_query(self, question: str, context: str = None) -> str:
        ...
        for line in context.split('\n'):
            ...
            if not line:
                continue
            if line.startswith('买家:') or line.startswith('买家：'):
                buyer_msg = line.split(':', 1)[-1].split('：', 1)[-1].strip()
                if buyer_msg:
                    buyer_messages.append(buyer_msg)
        merged_parts = [question] + buyer_messages[-3:]
        deduped = []
        for part in merged_parts:
            ...
            if not text or text in deduped:
                continue
            deduped.append(text)
        return '；'.join(deduped)[:300] if deduped else question
    ...
    def generate_reply(
        ...
        if is_repeated:
            logger.info(f"Detected repeated question, still using KB/context/order reasoning: {question[:50]}")
            ...
            if not matched_product_ids:
                ...
                if shop_products:
                    ...
                    for p in shop_products:
                        ...
                        if ppid and ppid in retrieval_query:
                            score = 1.0
                        else:
        ...
        kb_matches = self.knowledge_service.search_similar(
            retrieval_query, shop_id=shop_id, owner_id=owner_id, product_ids=matched_product_ids, limit=3
        )
        ...
        if not context and not knowledge_parts and not order_detail and not is_repeated:
        ...
        try:
            ...
            reply = self._call_openai(question, combined_context, order_detail, varied=is_repeated, model=model)
            ...
            if reply:
                ...
                if not context and not order_detail and not is_repeated:
                    cache_key = f"ai_reply:{md5_hash(question)}"
    ...
    def _call_llm(
        ...
        try:
            ...
            system_prompt = (
                ...
                "7. 使用亲切自然的语气，像真人客服一样交流，禁止使用'根据商品描述''暂无相关信息'等机械表述\n"
                "8. 回复控制在80字以内，必要时可两句话，不要太生硬\n"
                "9. 情绪感知：如果买家语气急躁、不满或抱怨，先用一句话真诚表示理解和歉意，再给出解决方案；语气友好则保持轻松愉快\n"
                "10. 如果买家重复提问，换种表达方式回答，并主动确认是否解决了问题\n"
                "11. 如果提供了订单信息，必须结合订单实际状态（支付状态、物流状态、快递单号、物流轨迹）来回答，不要泛泛而谈\n"
                "12. 禁止编造具体数值参数，不确定的数值说'建议您确认一下详情页哦'\n"
                "13. 禁止说'正在查询'、'请稍等'这类无意义的话，要么直接回答问题，要么询问买家需求\n"
                "14. 买家询问物流/快递/发货相关问题时，如果有快递单号和物流轨迹信息，直接告知具体单号和最新物流状态；没有物流信息时如实说明\n"
                "15. 话题切换：如果买家在对话中突然切换话题（如从退货转到询问优惠），自然跟随新话题回答，不要纠缠上一个问题\n"
                "16. 连环追问：买家连续追问同一问题时，直接回答根本原因，不要重复表面解释；必要时提供替代解决方案\n"
                "17. 竞品对比：买家拿其他平台或竞品比较时，不要贬低竞品，突出自身产品优势和当前优惠，引导关注产品本身价值\n"
                "18. 模糊问题：买家问题表述模糊或有歧义时（如'这个怎么样'），结合上下文猜测最可能的意图回答，完全无法判断则反问一个具体选项\n"
                "19. 禁止任何情况下推荐买家联系平台官方客服，所有问题由店铺自行解决或引导合理方案\n"
                "20. 仅退款处理：买家要求仅退款时，引导申请退货退款，回复'亲，关于退款问题，您可以申请退货退款哦~'"
            )
            ...
            if order_detail:
                ...
                for order in order_detail.get('orders', []):
                    ...
                    if order.get('trackingNumber'):
                        courier = order.get('courierName', '')
                        if courier:
                            order_lines.append(f"快递单号: {order['trackingNumber']}（{courier}）")
                        else:
                            order_lines.append(f"快递单号: {order['trackingNumber']}")
                    if order.get('logisticsTrajectory'):
                        order_lines.append(f"最新物流: {order['logisticsTrajectory']}")
